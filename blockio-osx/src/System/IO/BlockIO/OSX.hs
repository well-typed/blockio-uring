...
data IOCapCtx = IOCapCtx {
               ...
               -- | Locking of the writer end of the darwin ring: used by writers
               -- while they are preparing and submitting operations to the
...
-- is used and the workload of the program. There are benchmarks included in
-- this package's @benchmark/@ directory that can help gauge the performance for
-- a random 4k read workload, but some trial and error with custom benchmarks
-- might be required.
--
...
data IOCtxParams = IOCtxParams {
    ...
    --
    -- Accepted for API compatibility with @blockio-uring@; has no effect on
    -- Darwin (IOWAIT is a Linux @io_uring@ feature).
    ...
    mask_ $ do
      ...
      t <- forkOn capno $
             ...
             completionThread
               ...
               ioctxChanIOBatch
               ioctxChanIOBatchIx
      labelThread t ("System.IO.BlockIO.OSX.completionThread " ++
                     "(for cap " ++ show capno ++ ")")
...
-- these buffers are passed to @pread@\/@pwrite@ via GCD worker threads, and the
-- buffers must therefore not be moved around. 'submitIO' will check that
-- buffers are pinned, and will throw errors if it finds any that are not pinned.
...
prepAndSubmitIOBatch IOCapCtx {
    ...
    flip onException undoAcquisition $ do
      ...
      flip onException (putMVar ioctxURing muring) $ do
        ...
        -- undo / clear the pending operations that we prepared.
        URing.submitIO uring
...
data IOBatch = IOBatch {
                 ...
                 -- thread so that the buffers are kept alive while GCD worker
...
-- were GC'd, GCD worker threads could scribble on top of whatever got placed
-- there next). We reuse the original vector of IOOps that was submitted since this
...
completionThread !uring !done !maxc !qsem !chaniobatch !chaniobatchix = do
  ...
  where
    ...
    invalidEntry :: a
    invalidEntry =
      throw (UndefinedElement "System.IO.BlockIO.OSX.completionThread")
